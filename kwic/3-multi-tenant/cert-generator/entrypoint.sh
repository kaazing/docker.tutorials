...
CLIENT_CERT_SUBJECT="/C=US/ST=California/O=Kaazing/OU=Kaazing Demo"
...
function print_store()
{
  ...
  echo ""
  echo "------------------------------------------------------------------------------"
  echo "Listing of ${STORE}"
  echo "------------------------------------------------------------------------------"
  keytool -list -keystore ${STORE} -storepass ${STORE_PW} -storetype JCEKS
...
function create_server_cert()
{
  ...
  mkdir -p ${OUTPUT_DIR}/stores ${OUTPUT_DIR}/pkcs12
...
function create_client_cert()
{
  ...
  openssl req -new \
              ...
              -keyout ${CLIENT_KEY} \
              -subj "${CLIENT_CERT_SUBJECT}/CN=${CLIENT_ALIAS}" \
              -out ${CLIENT_CSR}
...
function create_server_stores()
{
  ...
  keytool -importcert -trustcacerts  -noprompt \
          ...
          -file ${CLIENT_ROOT_CERT} -alias ${CLIENT_CA_ALIAS} \
          -noprompt
  ...
  print_store ${KEYSTORE} ${KEYSTORE_PW}
  print_store ${TRUSTSTORE} ${TRUSTSTORE_PW}
}

#############################################################################

function create_client_stores()
{
  CLIENT_ALIAS=$1
  ...
  KEYSTORE=${OUTPUT_DIR}/stores/${KEYSTORE_NAME}-${CLIENT_ALIAS}.jceks
  KEYSTORE_PW_FILE=${OUTPUT_DIR}/stores/${KEYSTORE_NAME}-${CLIENT_ALIAS}.pw
  TRUSTSTORE=${OUTPUT_DIR}/stores/${TRUSTSTORE_NAME}-${CLIENT_ALIAS}.jceks
  TRUSTSTORE_PW_FILE=${OUTPUT_DIR}/stores/${TRUSTSTORE_NAME}-${CLIENT_ALIAS}.pw
  ...
  keytool -importcert -trustcacerts  -noprompt \
          -keystore ${TRUSTSTORE} -storepass ${TRUSTSTORE_PW} \
          -file ${SERVER_ROOT_CERT} -alias ${SERVER_CA_ALIAS} \
          -noprompt
...
function main()
{
  ...
  CLIENT_ROOT_CERT=${ROOT_CERT}
  create_client_cert tenant101
  create_client_cert tenant201

  create_server_stores
  create_client_stores tenant101
  create_client_stores tenant201
